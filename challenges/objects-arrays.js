...
const universities = [];
graduates.map(grads => universities.push(grads.university))
...
const contactInfo = [];
graduates.map(grads => {
  let name = grads.first_name;
  let email = grads.email;
  return contactInfo.push(`${name} ${email}`)
})
...
const unisWithUni = [];
graduates
  .filter(grads => grads.university.includes('Uni'))
  .map(unis => unisWithUni.push(unis))
...
let endangeredAnimals = zooAnimals.filter(animal => animal.population < 5)
lowPopulationAnimals.push(endangeredAnimals)
